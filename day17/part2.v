...
fn launch(mut velocity Velocity, target Target) bool {
...
fn main() {
	...
	for x in quad_bound(target.x0) .. target.x1 + 1 {
		for y in target.y1 .. int(math.abs(target.y1)) + 1 {
			mut velocity := Velocity{x, y}
			if launch(mut velocity, target) {
				count++
			}
		}
	}
